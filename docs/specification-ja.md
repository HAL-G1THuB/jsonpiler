...
`jsonpiler::functions::run` が返す終了コードは 256 を法としてラップされ、
0〜255 の値になります。
...
この言語は、JSON をベースとした構造に、軽量な構文糖と関数呼び出しや識別子による拡張を加えた形式です。トップレベルで `{}` に囲まれていないブロック形式 (`JSPL`) の構文も許容されます。
