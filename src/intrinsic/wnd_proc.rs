...
impl Jsonpiler {
  ...
  pub(crate) fn get_wnd_proc(&mut self, render: u32) -> ErrOR<u32> {
    ...
    self.data_insts.push(Seh(id, end_and_ignore, SIZE));
    self.data_insts.push(Seh(end_and_ignore, ignore_end, IGNORE_SIZE));
    self.insts.extend_from_slice(&[
      ...
      CallApiNull(screen_to_client),
      mov_q(Rcx, hwnd),
      LeaRM(Rdx, client_rect),
      CallApiNull(get_client_rect),
      ...
      mov_d(Operand::Sib(Sib { base: Rcx, index: R8, scale: S4 }, Disp::Zero), Rax),
      IncMd(pixel_x),
      Jmp(while_x),
      Lbl(while_end_x),
      IncMd(pixel_y),
