...
impl Jsonpiler {
  pub(crate) fn mov_args_json(
    ...
    let reg = *REGS.get(idx as usize).unwrap_or(&Rax);
    if !copy {
    ...
    for (tmp_idx, tmp_reg) in REGS.iter().enumerate() {
      if *tmp_reg != reg {
...
pub(crate) fn mov_imm(dst: Register, qword: i64) -> Inst {
  if qword == 0 {
    Clear(dst)
  } else if let Ok(dword) = i32::try_from(qword)
    && qword.is_positive()
  {
...
pub(crate) fn ret_memory(
  ...
  match mem_type {
    Size(size) => vec![if size == 1 { mov_b(addr, src) } else { mov_q(addr, src) }],
    Heap(None) => vec![mov_q(addr, src)],
    Heap(Some(size)) => {
...
pub(crate) fn mov_memory(dst: Register, Memory(addr, mem_type): Memory) -> Vec<Inst> {
  match mem_type {
    Size(size) => vec![if size == 1 { mov_b(dst, addr) } else { mov_q(dst, addr) }],
    Heap(None) => vec![mov_q(dst, addr)],
    Heap(Some(size)) => {
...
pub(crate) fn mov_memory_xmm(
  xmm: Register,
  tmp: Register,
  Memory(addr, size): Memory,
) -> Vec<Inst> {
  match size {
    Size(_) | Heap(None) => vec![MovSdM(xmm, addr)],
    Heap(Some(_)) => vec![mov_q(tmp, addr), MovSdRef(xmm, tmp)],
  }
}
pub(crate) fn ret_memory_xmm(
  ...
  match size {
    Size(_) | Heap(None) => vec![MovMSd(addr, xmm)],
    Heap(Some(_)) => vec![mov_q(tmp, addr), MovRefSd(tmp, xmm)],
  }
}
